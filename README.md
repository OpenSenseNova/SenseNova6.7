...
SenseNova 6.7 Flash 提供兼容 OpenAI API 格式的接口，可直接通过 HTTP 或 Python SDK 调用。
...
client = OpenAI(
    ...
    base_url="TODO_ENDPOINT_URL",  # TODO: 填入实际 endpoint
)

response = client.chat.completions.create(
    model="TODO_MODEL_NAME",  # TODO: 填入实际模型名
    messages=[
        {"role": "user", "content": "请分析以下数据并生成一份行业研究摘要：..."}
    ],
)
...
```bash
curl TODO_ENDPOINT_URL/chat/completions \
